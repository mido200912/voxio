...
const metaLogin = (req, res) => {
  ...
  // Removed pages_manage_metadata because the Meta App does not have it enabled
  const scope = 'pages_show_list,pages_read_engagement,instagram_basic,instagram_manage_messages,instagram_manage_comments';
...
const metaCallback = async (req, res) => {
  ...
  try {
    ...
    const tokenUrl = `https://graph.facebook.com/v20.0/oauth/access_token?client_id=${process.env.META_APP_ID}&redirect_uri=${BASE_URL}/api/integrations/meta/callback&client_secret=${process.env.META_APP_SECRET}&code=${code}`;
    const { data: tokenData } = await axios.get(tokenUrl);
    const shortLivedToken = tokenData.access_token;

    // 2. تبادل الرمز قصير الأجل برمز طويل الأجل (60 يومًا)
    const longLivedTokenUrl = `https://graph.facebook.com/v20.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${process.env.META_APP_ID}&client_secret=${process.env.META_APP_SECRET}&fb_exchange_token=${shortLivedToken}`;
    const { data: longLivedData } = await axios.get(longLivedTokenUrl);
    const userAccessToken = longLivedData.access_token;

    // 3. Get User's Pages
    const pagesUrl = `https://graph.facebook.com/v20.0/me/accounts?access_token=${userAccessToken}`;
    ...
    try {
      await axios.post(`https://graph.facebook.com/v20.0/${page.id}/subscribed_apps`, {
