...
if (mongoose.connection.readyState === 0) {
    ...
    mongoose.connect(mongoUri)
        ...
        .catch(err => console.error('❌ [FirestoreModel] MongoDB Connection Error:', err));
}
...
async function ensureIndexes(model, collectionName) {
    ...
    try {
        ...
        if (collectionName === 'users') {
            await model.collection.createIndex({ email: 1 }, { background: true, sparse: true });
            await model.collection.createIndex({ googleId: 1 }, { background: true, sparse: true });
        } else if (collectionName === 'companies') {
            await model.collection.createIndex({ owner: 1 }, { background: true });
            await model.collection.createIndex({ slug: 1 }, { background: true, unique: true, sparse: true });
            await model.collection.createIndex({ apiKey: 1 }, { background: true, sparse: true });
            await model.collection.createIndex({ chatToken: 1 }, { background: true, sparse: true });
        } else if (collectionName === 'company_chats') {
            await model.collection.createIndex({ company: 1, user: 1 }, { background: true });
            await model.collection.createIndex({ company: 1, createdAt: -1 }, { background: true });
        } else if (collectionName === 'integrations') {
            await model.collection.createIndex({ company: 1, platform: 1 }, { background: true });
        }
    } catch (e) {
        // Index creation is best-effort — don't crash on duplicate or permission errors
        if (e.code !== 85 && e.code !== 86) { // 85 = IndexOptionsConflict, 86 = IndexKeySpecsConflict  
            console.warn(`⚠️ Index creation warning for ${collectionName}:`, e.message);
        }
    }
}
